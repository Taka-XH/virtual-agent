  ...
  async (
    ...
    if (role !== 'user') {
      ...
      if (role === 'assistant' && text !== '') {
        try {
          const envVoice = process.env.NEXT_PUBLIC_SELECT_VOICE as
            | AIVoice
            | undefined
          const latestSettings = settingsStore.getState()
          if (envVoice && latestSettings.selectVoice !== envVoice) {
            settingsStore.setState({ selectVoice: envVoice })
          }
          console.log('External linkage speak settings:', {
            selectVoice: settingsStore.getState().selectVoice,
            voicevoxServerUrl: settingsStore.getState().voicevoxServerUrl,
          })
          // 文ごとに音声を生成 & 再生、返答を表示
          speakCharacter(
            sessionId,
